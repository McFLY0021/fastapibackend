...
def  Chofer_Valido(x:Chofer):
    ...
    if len(x.chapa)>7 or len(x.chapa)< 7:
...
def Mantenimiento_Valido(m:Mantenimiento):
    ...
    if len(m.chapa)< 7 or len(m.chapa)>7:
...
async def Existe_en_Bd(tabla:Table,dato_unico_comparable):
    conexion=SessionLocal()
    try :
        existe=conexion.query(tabla).filter(tabla.c.dato_unico_comparable == dato_unico_comparable).one()
        return True
